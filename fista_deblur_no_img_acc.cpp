...
void dct2(double* macroblock, double* dct2, int m, int n) {
    ...
    int i, j, k;
#pragma acc data copy(dct2[0 : m * n]) copyin(x[0 : m * n], y[0 : m * n], macroblock[0 : m * n])
{
    #pragma acc parallel loop gang private(i, j)
    for (i = 0; i < m; i++) {
        #pragma acc loop vector
        for (j = 0; j < n; j++) {
    ...
    #pragma acc parallel loop gang private(i, j, k, AlphaP, AlphaQ, sum, tmp)
    for (i = 0; i < m; i++) {
        #pragma acc loop vector
        for (j = 0; j < n; j++) {
            if (i == 0) {
...
void idct2(double* dct2, double* idct2, int m, int n) {
    ...
    int i, j, ii, jj;
#pragma acc data copy(idct2[0 : m * n]) copyin(x[0 : m * n], y[0 : m * n], dct2[0 : m * n])
{
    #pragma acc parallel loop gang private(i, j)
    for (i = 0; i < m; i++) {
        #pragma acc loop vector
    ...
    #pragma acc parallel loop gang private(i, j, AlphaP, AlphaQ, sum, tmp, ii, jj)
    for (i = 0; i < m; i++) {
        #pragma acc loop vector
        for (j = 0; j < n; j++) {
            sum = 0.0;
